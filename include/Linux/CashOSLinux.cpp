...
i32 OSRunShellProcess(ArrayView<const char*> args, AsyncData<std::string>* output, AsyncData<Path>* output_file, RunProcessFlags flags)
{
    // Redirect to /bin/sh
    std::vector<const char*> arg_array;
    arg_array.push_back("/bin/sh");
    arg_array.push_back("-c");
    for (const auto& arg : args)
        arg_array.push_back(arg);
    if (args.Last() != nullptr)
        arg_array.push_back(nullptr);

    return SysRunProcess(CreateArrayView(arg_array), output, output_file, flags);
}
